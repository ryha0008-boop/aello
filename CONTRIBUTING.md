...
- **Architecture lives in [CLAUDE.md](CLAUDE.md).** It documents every module (`src/` map), the blueprint/role model, the env-dir layout, auth, contextdb, and the release flow. Read the relevant section before changing behavior — it'll save you reverse-engineering.
...
Most logic (templates, placement, launch, role scaffolding) is unit-testable without ever launching Claude Code — see the `#[cfg(test)]` blocks in `src/`.
...
- **Keep docs in sync.** `README.md` for user-facing commands/roles; `docs/` for deeper reference (it's also the in-app help via `aello docs`, so a new `docs/*.md` appears automatically). Update whatever your change touches.
