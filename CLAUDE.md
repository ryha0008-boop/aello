...
**voice** (unconditional — every env) — a `Stop` hook that speaks each response's trailing `TL;DR:` line via Edge neural TTS, plus a `SessionEnd` group that releases the leased voice. Vendored from the `revoiced` project as three files copied together into `<env>/hooks/` (`hooks_speak.py` → `speak.py`, `hooks_duck.py` → `duck.py`, `hooks_win_audio.ps1` → `win_audio.ps1`): `speak.py` does `sys.path.insert(HERE)` + `import duck` and shells out to `HERE/win_audio.ps1`, so a partial copy fails at runtime. Upstream `speak.py` also imports `focus` and `notify`, but **guarded** (`try`/`ImportError` → `SimpleNamespace` stubs, added upstream in `a86023a` for this exact reason): those two are station-side and are deliberately **not** vendored, so the toast/window-tracking degrades to its non-Windows behaviour here. Vendor the three, never the five. Registered as `$CLAUDE_CONFIG_DIR/hooks/speak.py` — **never a path to a checkout**, which is the whole point: that coupling silences every env at once when one directory moves. `sync_voice_hooks` self-heals registration into an already-placed env (register-only — there is no deregister branch, since no env is ever made silent by placement), replacing a hand-wired absolute-path group rather than adding beside it, and `ensure_tldr_instruction` appends the TL;DR section to the persona (append-only — the hook speaks that line and nothing else, so a persona that never writes one is silent). The scripts' state (voice pool, per-session leases, mutes) is **machine-wide**, not per-env: `%LOCALAPPDATA%\revoiced` / `~/Library/Application Support/revoiced` / `$XDG_DATA_HOME/revoiced`. That's what makes concurrent envs lease different voices and queue behind one playback lock. `voice.rs` (`aello voice mute|unmute|stop|status`) writes that shared state directly — no Python, no placed env, works from any directory — preserving unknown keys since the hook owns the rest of the file.
...
- `voice.rs` — `aello voice mute|unmute|stop|status`; `data_dir` mirrors `speak.py`'s per-OS data dir, read/write of the shared `state.json` (preserving unknown keys), and the `run/stop` token that halts a sentence already playing.
- `update.rs` — self-update from the rolling `latest` release.
- `src/hooks_speak.py`, `src/hooks_duck.py`, `src/hooks_win_audio.ps1` — the vendored `voice` hook trio (from `revoiced`), copied into `<env>/hooks/` only when the cap is on. Re-sync them from upstream as a unit.
