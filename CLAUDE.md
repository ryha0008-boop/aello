...
2. **build** jobs (ref: the bump commit sha, so the binary reports the new version) build `x86_64-unknown-linux-gnu` → `aello-x86_64-linux` and `x86_64-pc-windows-msvc` → `aello-x86_64-windows.exe`.
