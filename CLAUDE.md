...
**Capabilities** (`models.rs::Capabilities`) — six bools chosen at `add` time: `project_md`, `github`, `changelog`, `docs`, `readme`, `voice`. `#[serde(default)]` so old configs load all-false. Each enabled cap, on placement, scaffolds its file (only if missing) and contributes a section to a generated `/sync` skill. **`voice` is the exception** — it maintains no project file and adds no `/sync` section, so `Capabilities::any()` (the gate for seeding `/sync`) deliberately excludes it; a voice-only blueprint must not get an empty `/sync`. `github` scaffolds the most: the `.claude-env-*` gitignore line, `.gitattributes` (CRLF normalize), a generic `VERSION` + stack-agnostic `.github/workflows/version.yml` patch-bump CI, and the tracked `claude-internal/` mirror (see below).
