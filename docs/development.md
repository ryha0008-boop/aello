...
1. **test** — `cargo test` on `ubuntu-latest` **and** `windows-latest`, and everything else needs it, so a red suite blocks the release *and* the version bump. It gates the bump deliberately: a bump commit pushed for a build that then fails leaves a version that was never released, and the next push bumps straight past it. Both platforms because the suite had previously only ever run where a human ran it — Windows — and had **never** executed on Linux, which is where the cross-platform assumptions actually break: path separators, case sensitivity, and the line-ending normalisation the hook-digest test does explicitly so a Windows checkout and Linux CI can agree. macOS is not covered on purpose; it builds from source only. It runs on the pushed commit rather than the bump commit, since the bump changes only a version string.

   **Do not add `--locked`.** The bump job commits a new `Cargo.toml` version without touching `Cargo.lock`, so from the next push onward the lockfile's recorded package version trails by one and `--locked` fails every run on a mismatch that means nothing.
2. **bump** — resolves the version. If the version in `Cargo.toml` already has a `vX.Y.Z` tag, it increments the patch, commits `release: vX.Y.Z [skip ci]` and pushes via `GITHUB_TOKEN` (which does not re-trigger CI). If it has no tag yet, a human set it deliberately and it is published unchanged — that is how a minor or major bump ships, since a patch bump can only ever produce X.Y.(Z+1).
3. **build** — four targets, at the bump commit's sha so the binary reports the new version: `x86_64-unknown-linux-gnu`, `x86_64-pc-windows-msvc`, `aarch64-apple-darwin`, and `x86_64-apple-darwin` (cross-compiled on the Apple Silicon runner). All unsigned — `install.sh` clears the macOS quarantine attribute and the README documents Windows SmartScreen.
4. **publish** — generates `SHA256SUMS` and uploads binaries + manifest to **two** releases: the immutable `vX.Y.Z` tag (flagged `--latest`) and the permanent rolling `latest` tag (`--clobber`, explicitly `--latest=false`).
