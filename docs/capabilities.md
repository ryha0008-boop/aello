...
The global persona (`--claude-md`) is separate from capabilities — it writes the env-level `CLAUDE.md` once. So is the voice, below: it is not a capability and there is nothing to enable.

## The voice — every env speaks

Every placed env gets `hooks/speak.py` + `duck.py` + `focus.py` + `notify.py` + `win_audio.ps1`, a `Stop` hook that speaks each response's trailing `TL;DR:` line through a free Edge neural voice, a `SessionEnd` hook that hands the borrowed voice back, and a TL;DR section appended to its persona.

**It used to be a capability (`--voice`) and no longer is.** Choosing it per blueprint bought nothing: a blueprint that maintains no project file has no reason to be mute, and the one time you actually want silence — right now, because a machine started talking — is a runtime decision that a placement flag answers far too slowly. So the flag is gone, along with `--no-voice`, and silence lives entirely in `aello voice mute`. An existing config's `voice = …` is ignored on load and dropped on the next save.

**Why the hook is vendored into the env.** It's copied to `<env>/hooks/` and registered as `$CLAUDE_CONFIG_DIR/hooks/speak.py`. The alternative — pointing every env at one checkout of the script — couples unrelated projects to that path: move or rename it and every env goes silent at once, and each newly placed env stays silent until edited by hand. `speak.py` imports `duck`, `focus` and `notify` as siblings and shells out to `win_audio.ps1` beside it, so all five are copied together.

**Why all five, when two of them are optional.** `speak.py` wraps the `focus` and `notify` imports in `try`/`ImportError` so a partial copy still speaks instead of dying on every response. That guard once made aello ship three of the five, and the result was worse than a crash: every env spoke normally while `notify`'s stub quietly returned "not shown", so **no desktop notification was raised anywhere** and nothing said so. Guarded means a partial copy survives, not that the file is optional — vendor all five. `speak.py --status` now leads with `INCOMPLETE COPY - no …` when it is missing a sibling, so the same failure can't be silent twice. One caveat survives a complete copy: on Windows a toast is shown *as* a registered application, and one sent under an unregistered identity is dropped without an error — so the notification appears where the revoiced station has registered its own identity, and nowhere else. Speech is unaffected either way.

**Who registers the toast identity.** That caveat is now handled: launching an env runs the vendored `notify.py --register` — no test toast, idempotent — so the toasts appear on a machine that has never started the station. Registration has two halves and they belong to different owners. The **identity** is just a name a toast is shown under, and any copy may claim it. The **`revoiced:` protocol** is a promise to run something — it points at an `action.py` beside whichever copy registered it, and an env's `hooks/` has none, since aello vendors the five hook-path files and not the station. Upstream splits them accordingly (`HOOK_VERSION = 5`): a vendored copy claims the identity and leaves the protocol alone, so it can't take a working toast button away from the station. Before that split, registering from an env would have repointed both buttons at a missing file on every launch.

**Knowing when the copy is behind.** `speak.py` carries a `HOOK_VERSION`, bumped upstream whenever one of the five files changes. `aello voice status` prints the version aello vendored, and `python <env>/hooks/speak.py --status` prints the version that env actually runs — if the two disagree, that env is behind and a `run` will refresh it. Two unit tests keep the vendored copy honest: one compares `HOOK_VERSION` against the constant in the vendored `speak.py`, and one digests **all five** files, since the constant lives in `speak.py` alone and a re-vendor touching only `duck.py` or `win_audio.ps1` would otherwise slip past.
...
**The persona has to cooperate.** The hook speaks the `TL;DR:` line and nothing else, so placement appends a section to the env's `CLAUDE.md` telling it to end every response with one. It's appended, never rewritten, so an env placed before this existed picks the section up without disturbing a persona you've edited. A response with no TL;DR is blocked once with a request to add one, then allowed through — it can't loop.

**Migrating a hand-wired hook.** If an env already has a `Stop` hook you added yourself pointing at a checkout (`python "C:/…/revoiced/speak.py"`), placement **replaces** it with the env-relative one on the next `run`. It isn't added beside it — that would speak every response twice — and it isn't left alone, since that absolute path is the coupling vendoring exists to remove. Hooks that aren't a `speak.py` are never touched.

**Turning it off.** `aello voice mute` (or `mute --project`, `stop`, `status`) — it writes the shared state directly, so it needs neither Python nor a placed env, works from any directory, and applies to every env at once. The TUI has the same switch on `M`, which toggles the global mute and shows `VOICE: MUTED` in the footer while it's on. There is no way to make a single env permanently silent by placing it differently, and that is deliberate — mute is reversible in one keystroke, a placement flag is not.
