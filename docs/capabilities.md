...
**Knowing when the copy is behind.** `speak.py` carries a `HOOK_VERSION`, bumped upstream whenever one of the five files changes. `aello voice status` prints the version aello vendored, and `python <env>/hooks/speak.py --status` prints the version that env actually runs — if the two disagree, that env is behind and a `run` will refresh it.
