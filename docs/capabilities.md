...
Every placed env gets `hooks/speak.py` + `duck.py` + `win_audio.ps1`, a `Stop` hook that speaks each response's trailing `TL;DR:` line through a free Edge neural voice, a `SessionEnd` hook that hands the borrowed voice back, and a TL;DR section appended to its persona.
