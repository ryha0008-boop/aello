...
**Who registers the toast identity.** That caveat is now handled. Launching an env runs the vendored `notify.py --register` — protocol plus toast identity, no test toast, idempotent — but only when nothing has claimed the `revoiced:` protocol yet. Registering also points that protocol at an `action.py` next to whichever copy ran it, and an env's `hooks/` has none: aello vendors the five hook-path files, not the station. Doing it unconditionally would repoint both toast buttons at a missing file on every launch, on exactly the machines where a station had them working. An unclaimed protocol means no station has ever run here, which is the case worth covering — the buttons need a station to talk to anyway, and the station reclaims the protocol the first time it starts.
