...
**The cost of passing it as an environment variable: Claude Code cannot see your subscription tier.** Measured 2026-09-13 against Claude Code 2.1.263. When `CLAUDE_CODE_OAUTH_TOKEN` is set, the CLI builds its account record from the variable alone — `subscriptionType` and `rateLimitTier` come from `CLAUDE_CODE_SUBSCRIPTION_TYPE` / `CLAUDE_CODE_RATE_LIMIT_TIER` or are `null`, and the function that would otherwise read the real values from local account state returns early precisely *because* the variable is set. With the tier unknown, tier-gated features go dark: the header reads **Claude API** instead of Claude Max and **Opus with 1M context disappears from `/model`**. Declaring both variables restores both — same token, same binary. (On 2.1.263 the Fable row went missing too; on 2.1.282 a fresh env lists Fable with or without a plan, so that part was fixed upstream.)

**So aello asks.** The token cannot answer this — a `setup-token` credential carries only the `user:inference` scope, so the profile endpoint refuses it (403, `user:profile` required), and inference responses carry quota headers but no plan name (measured 2026-09-25). `aello login` asks which plan the login is on right after storing the token, and an install that logged in before the question existed is asked once on its next `aello run` from a terminal. The answer is `plan` in `config.toml`, changed any time with `aello plan <pro|max-5x|max-20x|team|enterprise|unknown>`; each launch turns it into the two variables, and removes any inherited copies first, so a parent env or your shell cannot stand in for it. `unknown` sets nothing, which is the behaviour before this existed. Verified on 2.1.282 against the real token: `max-5x` gives the Claude Max header and the Opus 1M default; `unknown` on the same env gives Claude API and neither.

What this does **not** affect is model access or your default model. An env's default comes from its `settings.json` (`"model": "opus"`), and a model missing from the picker is still selectable by name: `/model fable` in a session, or `--model fable` at launch, both work on a tier-blind token and were verified end to end (the served request records `claude-fable-5-1`). The token being *shared* is not the cause — a per-session token exported the same way behaves identically.
...
**Why the transcript is copied and not just referenced.** It used to be a path only. Claude Code deletes its own session files after `cleanupPeriodDays` — **default 30** — and the env dir holding them is gitignored and removed outright by `aello remove --purge`. So the reference silently stopped resolving: in that same audit, 15% of archives already pointed at nothing, with a clean cliff at the 30-day mark (6–14% dead under 30 days, 44% at 30–39). Nothing errored; the archive just quietly stopped being one. **Since 2026-08-12 the original is then deleted**, when the copy verifies (sha256, both directions) *and* the session wrote a `/handoff` note — contextdb becomes the single store, and Claude Code's copy, the one sitting inside a working tree, is gone within the session. The note is the gate because deleting the original costs `--resume` for that session and nothing else: the archive holds every byte, and a handoff note *is* the continuity, so once it exists the transcript's resume value is spent. Without a note the original stays. `cleanupPeriodDays` accordingly went from 365 to **10** and now migrates in existing envs (a value you chose yourself is still left alone) — ten rather than three because an unarchived transcript belongs to a session that never reached SessionEnd, which is the one you might still want to resume.
