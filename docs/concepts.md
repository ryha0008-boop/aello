...
- **Global / persona** — `<env>/CLAUDE.md`. The agent's identity ("you are a coding agent…"). Chosen with `--claude-md`: `coder` for a coding project, `none` for anything else, or a path to a file you maintain. Becomes `custom` once a generated persona is accepted, after which aello writes nothing over it. Written once; never overwritten on later runs.
